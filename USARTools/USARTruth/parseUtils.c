...
while ((*ptr != 0) && (*ptr != '{')) ptr++; /* find the { */
if (*ptr == 0) return msg;	/* didn't find the { */
ptr++;	 /* skip over the { */
while (isspace(*ptr)) ptr++;	/* skip over any space */
...
while ((isspace(*ptr)) || (*ptr == ',')) ptr++; /* skip over delimiters */

/* copy everything until we hit a delimiter */
while (! isspace(*ptr)) {
if (*ptr == 0) return ptr;	/* didn't finish cleanly */
if ((*ptr == ',') ||
(*ptr == '}')) {
break;
}
...
nextptr = getValue(ptr, token);	
if (nextptr == msg) return msg;	
if (1 != sscanf(token, "%i", intValue)) return msg;	
ptr = nextptr;
return ptr;
}
...
nextptr = getValue(ptr, token);	
if (nextptr == ptr) return msg;	
if (1 != sscanf(token, "%lf", dblValue)) return msg;	
ptr = nextptr;
return ptr;
}
...
if( foundParent && foundLoc && foundRot ) return ptr;
}
