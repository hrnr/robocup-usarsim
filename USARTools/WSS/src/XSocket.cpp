...
void XSocket::init()
{
	...
	if( sockfd >=0  ) {
		...
		if (setsockopt(sockfd,SOL_SOCKET,SO_NOSIGPIPE,&yes,sizeof(int)) == -1) {
			ERR( "Could not set SO_NOSIGPIPE on socket! %s", lastError().c_str() );
		}
	}
#endif
}
...
bool XSocket::connect(const string& host, unsigned int port)
{
    ...
    if( ::connect(sockfd, (sockaddr*)&remoteAddr, sizeof(remoteAddr)) == -1 ) {
  		ERR( "Couldn't connect to %s:%i! (sock:%d) (%s)", host.c_str(), port, (int)sockfd, lastError().c_str() );
		destroySocket();
		return false;
  	}
...
int XSocket::read(char* buf, int bufLen, unsigned long timeout) {
	...
	if( !aIsConnected ) {
		ERR("Can't read from a disconnected socket!");
		return -1;
	}
	
	if(timeout > 0 && !waitForData(timeout)) {
			return -2;
	}
	...
	{
		aIsConnected = false;
		DBG( "Couldn't read data! (%s)", lastError().c_str() );
		return -1;
	}
	if( numNewlyRead == 0 ) {
...
int XSocket::write(const char* buf, int bufLen) {
	...
	if( !aIsConnected ) {
		ERR("Can't write to a disconnected socket!");
		return -1;
	}
	
#if !defined(__IS_WINDOWS__) && !defined(__APPLE__)
	const int flags = MSG_NOSIGNAL;
#else
	const int flags = 0;
#endif
	
	while(numWrittenEver < bufLen) {
		if( ( numWritten = ::send(sockfd, buf+numWrittenEver, bufLen-numWrittenEver, flags) ) == -1 ) {
			aIsConnected = false;
...
int XSocket::send(string toBeSent, string delim)
{
	...
	{
		if( ( numWritten = ::send( sockfd, charData+numWrittenEver, charDataLen-numWrittenEver, flags) ) == -1 )
		{
			destroySocket();
			ERR( "Couldn't send data! (%s)", lastError().c_str() );
			return numWrittenEver;
